...
function WorkshopGenerate({ setCreateWorkshop, isHaveCharacter }) {
  ...
  useEffect(() => {
    if (isHaveCharacter === "true") {
      axios
        .get("http://localhost:9000/api/user/v4/getallicname")
        .then((response) => {
  ...
  const submitGenerate = () => {
    if (workshopName === "" || workshopId === "") {
      ...
      axios
        .post("http://localhost:9000/api/user/v4/createworkshop", data)
        .then((response) => {
          if (response.data.error) {
            toast.error(response.data.message);
          } else {
            if (
              response.data.message === "Már van egy műhely a tulajdonodban."
            ) {
              toast.error(response.data.message);
            } else {
  ...
  return (
    <>
      <motion.h2 onClick={showModal} whileHover={{ scale: 1.1 }}>
        ...vagy adj hozzá újat.
      </motion.h2>
      <Modal
        title="Műhely létrehozás"
        footer={null}
        open={isModalOpen}
        onOk={handleOk}
